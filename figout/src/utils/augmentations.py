"""
 ||------------------------------------------------------\\
||     This originally comes from TensorFlower Garden:    \\
...
class ImageAugment(object):
    ...
    def augment_labelled(self, image: tf.Tensor,
...
def get_augment_layers():
    """Temporary holder for custom augmentation layers. TODO: Develop into ImageAugment object"""
    augment_layers = [
            ...
            tf.keras.layers.experimental.preprocessing.RandomFlip(
                mode="horizontal_and_vertical", seed=123,
            ),
            tf.keras.layers.experimental.preprocessing.RandomTranslation(
                (-0.2, 0.2), (-0.2, 0.2), fill_mode='reflect',
                interpolation='bilinear', seed=123, fill_value=0.0
            )
        ]

    def augment(image, label):
        for augmenter in augment_layers:
